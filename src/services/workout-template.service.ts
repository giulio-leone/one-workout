import { logError, logger as sharedLogger, toPrismaJsonValue } from '@giulio-leone/lib-shared';
import { createId, prisma } from '@giulio-leone/lib-core';
import { hasValidSetGroups } from '@giulio-leone/one-workout';
import type {
  WorkoutTemplate,
  WorkoutTemplateType,
  Exercise,
...
const serviceLogger = sharedLogger.child('WorkoutTemplate');
 ...
 * SSOT: Usa setGroups come unica fonte di verità per le serie
 */
function validateTemplateData(
  ...
  switch (type) {
    case 'exercise': {
      ...
      // SSOT: Valida setGroups invece di sets
      if (!hasValidSetGroups(exercise)) {
        throw new Error("L'esercizio deve contenere almeno un gruppo di serie (setGroups)");
      }
      break;
    }
    case 'day': {
      ...
      // Valida che tutti gli esercizi abbiano setGroups validi
      for (const exercise of day.exercises) {
        if (!hasValidSetGroups(exercise)) {
          throw new Error(`L'esercizio "${exercise.name}" deve contenere setGroups validi`);
        }
      }
      break;
    }
    case 'week': {
      ...
      // Valida tutti i giorni e i loro esercizi
      for (const day of week.days) {
        if (!day.exercises || day.exercises.length === 0) {
          throw new Error(`Il giorno ${day.dayNumber} deve contenere almeno un esercizio`);
        }
        for (const exercise of day.exercises) {
          if (!hasValidSetGroups(exercise)) {
            throw new Error(
              `L'esercizio "${exercise.name}" nel giorno ${day.dayNumber} deve contenere setGroups validi`
            );
          }
        }
      }
...
export class WorkoutTemplateService {
  ...
  static async createTemplate(
    ...
    const template = await prisma.workout_templates.create({
      data: {
        ...
        tags: data.tags || [],
        data: toPrismaJsonValue(JSON.parse(JSON.stringify(data.data))),
  ...
  static async listTemplates(
    ...
    try {
      ...
      logError('listTemplates error', error);
      serviceLogger.debug('Query details', { where, orderBy, options });
      throw error;
    }
  }
  ...
  static async updateTemplate(
    ...
    if (data.data !== undefined) {
      updateData.data = toPrismaJsonValue(JSON.parse(JSON.stringify(data.data)));
    }
